...
- **地图管理**：方形空间地图，可自定义大小
- **多种控制算法**：
  - **算法1：先旋转再前进** - 先调整角度，再直线前进（适合精确导航）
  - **算法2：边旋转边前进** - 同时旋转和前进，智能选择前进/后退（适合快速导航）
  - **算法3：PID控制** - 双PID+积分抗饱和，精度高、稳定
  - **算法4：Pure Pursuit** - 纯跟踪算法，适合路径跟踪
  - **算法5：Stanley** - 横向+航向误差联合控制，精度最优
  - **算法6：滑模/切换控制** - 基于论文的相位切换，前进/后退/转向结合，**保证任意(x,y,θ)可达**（推荐）
...
程序使用 **传统4轮Ackermann转向模型**（标准物理模型）：
- 以后轴中心为参考，考虑轴距和转向角约束
- 符合前轮转向、后轮驱动的实际机制
- **无法原地转向**，低速转向采用最小蠕行速度近似
